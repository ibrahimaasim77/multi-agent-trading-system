...
financial:
  ...
  guardrail_aborts: 8              # 6/25 morning+intraday, 6/26 morning+intraday, 6/29 morning+intraday, 6/30 morning+intraday
  # note (updated 2026-06-30): account 912269602 (Agentic, ••••9602) has now
  # shown $0.00 total_value / $0.00 cash for FOUR consecutive trading days
  # (6/25, 6/26, 6/29, and 6/30 — 6/27/6/28 were the weekend), down from
  # $217.19 cash_close recorded in the 2026-06-24 journal, with ZERO trades
  # placed and ZERO orders ever recorded on this account (checked across all
  # time). Twelve independent live get_portfolio queries across four trading
  # days have all returned identically $0.00. Two full weekends have elapsed
  # with no self-correction. No order, transfer, or withdrawal record is
  # visible to any agent that explains the drop from $217.19 to $0.00. This
  # is a CONFIRMED, PERSISTENT capital-layer failure — not a provisional
  # anomaly. Could be an unauthorized withdrawal, an ACH sweep, or a
  # portfolio-endpoint data bug — cannot be determined from inside this system.
  # The pnl/account-value fields above are computed mechanically from the
...
benchmark:
  ...
  spy_close_today: 746.33             # EOD 2026-06-30 (end of Q2)
  spy_pct_change_since_start: +0.26
  system_alpha_vs_spy_pct: -100.26   # UNCONFIRMED — mechanical result of the unexplained $0 balance, not a skill signal. See financial note above.
```
